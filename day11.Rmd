...
The brain of the robot is the Intcode computer and the provided program.  For this puzzle the challenge is to add actions that the "brain" wants to do.

The intructions say, "The robot will continue running for a while like this and halt when it is finished drawing."  This confused me as I did not see a `99` in the `painting_pgm`.  It turns out the programs creates it while it executes.
...
Cycles through the 4 possible directions.  
...
Robot <- 
  ...
  public = list(
    brain = NULL, 
    facing = NULL,  # direction it is facing, 0:3
    pos_id = NULL, # id of robot's position; easier than tracking coordinates
    ...
    paint = function(col) { # using output[1] from brain
    ...
    move = function(dir) { # using output[2] from brain
...
# Create the Robot and Start It Painting

```{r}
painting_robot <- Robot$new(paint_pgm)
painting_robot$start(0)
```

# Start the report on a white panel

```{r}
painting_robot <- Robot$new(paint_pgm)
painting_robot$start(1)

ggplot(data = painting_robot$hull, aes(x = x, y = y, fill = as.factor(color))) + 
  geom_tile() +
  scale_y_reverse() + 
