...
# Wire is constructed from a vector of strings defining the legs
wire <- function(string) {
  string %>% str_split(",") %>% unlist()
}


# wire_path, the path of the wire is in a data frame with columns x and y

# Adds a leg the wire_path built so far
path <- function(leg, origin) {
    
    direction <- str_sub(leg, 1, 1)
    distance <- as.numeric(str_remove(leg, "^."))
    
    if(direction == "R") {
      path <- list(c(1:distance), c(rep(0, distance)))
    }
    else if(direction == "L") {
      path <- list(c(-1:-distance), c(rep(0, distance)))
    }
    else if(direction == "U") {
      path <- list(c(rep(0, distance)), c(1:distance))
    }
    else {
      path <- list(c(rep(0, distance)), c(-1:-distance))
    }
    return(
      tibble(x = origin["x"] + path[[1]], 
             y = origin["y"] + path[[2]])
    )
    # return(path)
}
...
# Each new leg of wire path requires a new origin
new_origin <- function(df) {
  tail(df, 1) %>% unlist()
}

# Builds to the set of cooridnates for wire
wire_path <- function(wire) {
  wire_path <- tibble(x = 0, y = 0)
  for(leg in 1:length(wire)) {
    wire_path <- bind_rows(wire_path, path(wire[leg], new_origin(wire_path)))
  }
  return(wire_path)
}
...
#### Easy way to find multiple visits to the same place
    group_by(x, y) %>% mutate(step = min(step)) %>% ungroup()
}

timing_distance <- function(wire1, wire2) {
  wire_path1 <- wire_path(wire1)
  wire_path2 <- wire_path(wire2)
  inner_join(wire_path1, wire_path2, by = c("x", "y")) %>%

#### .x and .y are added to common variables not used in the join
    mutate(t_distance = step.x + step.y) %>% 
    filter(t_distance > 0) %>% 
    filter(t_distance == min(t_distance))
}  

# tests
wire1 <- wire("R75,D30,R83,U83,L12,D49,R71,U7,L72")
wire2 <- wire("U62,R66,U55,R34,D71,R55,D58,R83")
timing_distance(wire1, wire2)
...
timing_distance(wire1, wire2) # took 2.21 seconds
