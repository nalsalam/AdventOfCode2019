                 ...
                 run = function(input) {
                   ...
                   while(TRUE) {
                     ...
                     if(Opcode == 1) {
                       ...
                       if(is.na(self$pgm[values[1]]) || is.na(self$pgm[values[2]])) {
                         print(paste(self$ptr, self$base, Opcode, modes, values, self$pgm[values[1]], self$pgm[values[2]]))
                       }
                       if(self$pgm[values[1]] == self$pgm[values[2]]) self$pgm[values[3]] <- 1
                       else self$pgm[values[3]] <- 0
                       self$ptr <- self$ptr + 4 
...
=======
### Quine computer - ## copies itself but need to modify to keep building output until it halts
>>>>>>> 78671f15853006b99ddf42607724e59dca2c489d
# no phase, so commented that out
# no input so no worries about that parameter or the pause behavior

# for some reason these can't be run twice

Quine <- Amp$new(
  c(109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99, rep(0, 150)), 
  phase = 0)
Quine$run(0)
...
pgm <- c(109,19,204,-34, 99, rep(0, 2100))
pgm[1986] <- 66
ptr <- 0
base <- 2000

## copies itself but need to modify to keep building output until it halts
pgm <- c(109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99, rep(0, 150))
ptr <- 0
base <- 0
output <- NA

Opcode <- pgm[ptr + 1] %% 100
...
modes <- 
  pgm[ptr + 1] %/% (10 * 10^c(1:3)) %% (10^c(1:3)) # vector of 0, 1, 2's
values <- 
  dplyr::case_when(
    modes == 0 ~ c(pgm[ptr + 2] + 1, pgm[ptr + 3] + 1, pgm[ptr + 4] + 1), # position mode
    modes == 1 ~ c(ptr + 2, ptr + 3, ptr + 4), # immediate mode
    modes == 2 ~ c(pgm[ptr + 2] + 1 + base, pgm[ptr + 3] + 1 + base, pgm[ptr + 4] + 1 + base), # relative mode
    TRUE ~ NA_real_
  )

if(Opcode == 1) {
  pgm[values[3]] <- pgm[values[1]] + pgm[values[2]]
  ptr <- ptr + 4
  ...
  if(pgm[values[1]] != 0) ptr <- pgm[values[2]] 
  else ptr <- ptr + 3
  ...
  if(pgm[values[1]] < pgm[values[2]]) pgm[values[3]] <- 1
  else pgm[values[3]] <- 0
  ptr <- ptr + 4
  ...
  base <- pgm[values[1]] + base  
  ptr <- ptr + 2 
  
} else {
  print(paste("Invalid instruction at position ", ptr))
}
